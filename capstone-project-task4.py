...
If both players and the dealer get the same value of cards , it's a TIE and the bet money is returned.
If none of the above cases are met ,the person with closer value to 21 wins.
(winning like this returns 2x the bet money)

Buying insurance:
Players are asked whether to buy insurance if the deal's shown card is Ace. If players choose to buy insurance,
they need to place another insurance bets equal to half of the initial bets. If the dealer gets blackjack from 
the first two cards (dealer wins) and players decide to buy insurance, they will win the twice insurance bet. 
Essentially, buying insurance has the probability of helping players reduce the loss to the half of initial bet 
when the dealer gets blackjack from the first two cards.

Double down:
Players have options to double down (place another bet equal to the initial) bet after getting the first two 
cards. Then players can only get one card from the dealer and stop their game decisions.

Split cards:
Players have options to split their cards when the first two cards have the same rank (regardless of suits). 
When splitting cards, players have to place another bet equal to the initial bet and have two hands to play the 
game. The dealer will distribute additional two cards to each hand and ask for player's game decisions 
(double down/hit/stand) to continue the game.
-----------------------------------------------------------------------------------------------------------------
